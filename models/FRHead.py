...
class FRPredictHead_FCOS(nn.Module):
    def __init__(self, way, shot, Woodubry):
        ...
        self.r = nn.Parameter(torch.zeros(2), requires_grad=True)
        self.scale = nn.Parameter(torch.FloatTensor([1.0]), requires_grad=True)
        self.distance_threshold = nn.Parameter(torch.FloatTensor([1.0]), requires_grad=True)
    ...
    def cls_predictor(self, support: torch.Tensor, boxes_features: torch.Tensor, Woodubry=True):
        ...
        euclidean_matrix, background_index, foreground_index = self.compute_background(euclidean_matrix)
        metric_matrix = self.metric(euclidean_matrix,
                                    box_per_image=roi_num,
                                    resolution=self.resolution)  # (roi数, way)
        logits = metric_matrix * self.scale.exp()  # (roi数, way + 1), 防止logits的数值过小导致softmax评分差距不大, 温度T
    ...
    def compute_background(self, euclidean_matrix):
        background_metrix = torch.ones_like(euclidean_matrix).to(euclidean_matrix.device) * 114514
        euclidean_matrix = torch.where(euclidean_matrix > self.distance_threshold, background_metrix, euclidean_matrix)
        background_index = torch.where(euclidean_matrix.mean(1) >= 114514)[0]
        foreground_index = torch.where(euclidean_matrix.mean(1) < 114514)[0]
        assert len(background_index) + len(foreground_index) == euclidean_matrix.shape[0]
        return euclidean_matrix, background_index, foreground_index
...
class FRPredictHead_FRDet(nn.Module):
    ...
    def compute_background(self, euclidean_matrix):
        background_metrix = torch.ones_like(euclidean_matrix).to(euclidean_matrix.device) * 114514
        euclidean_matrix_mask = torch.where(euclidean_matrix > self.distance_threshold,
                                            background_metrix, euclidean_matrix)
        ...
        bg = torch.zeros([n, 1]).to(euclidean_matrix.device)
        bg[foreground_index] = self.distance_threshold * 3
        euclidean_matrix = torch.cat([bg, euclidean_matrix], dim=1)
        return euclidean_matrix, background_index, foreground_index
