...
class FRPredictHead_Simple(nn.Module):
    def __init__(self, way, shot, representation_size, num_classes, Woodubry):
        super(FRPredictHead_Simple, self).__init__()
        self.resolution = None
        self.way = way
        self.shot = shot
        self.representation_size = representation_size
        ...
        self.encoder = nn.Sequential(nn.Conv2d(256, 64, kernel_size=1, stride=1, padding=0),
                                     ...
                                     nn.LeakyReLU(inplace=True))
        self.r = nn.Parameter(torch.zeros(2), requires_grad=True)
        self.scale = nn.Parameter(torch.FloatTensor([0.5]), requires_grad=False)
...
class FRPredictHead_FCOS(nn.Module):
    def __init__(self, way, shot, Woodubry):
        super(FRPredictHead_FCOS, self).__init__()
        self.resolution = None
        self.way = way
        self.shot = shot
        ...
        self.r = nn.Parameter(torch.zeros(2), requires_grad=True)
        self.scale = nn.Parameter(torch.FloatTensor([1.0]), requires_grad=True)
        self.distance_threshold = nn.Parameter(torch.FloatTensor([0.5]), requires_grad=False)
    ...
    def cls_predictor(self, support: torch.Tensor, boxes_features: torch.Tensor, Woodubry=True):
        ...
        metric_matrix = self.metric(euclidean_matrix,
                                    box_per_image=roi_num,
                                    resolution=self.resolution)  # (roi数, way)
        logits = metric_matrix * self.scale.exp()  # (roi数, way), 防止logits的数值过小导致softmax评分差距不大, 温度T
        return logits, support
    ...
    def metric(self, euclidean_matrix, box_per_image, resolution):
        ...
        #     .mean(1)  # (roi数, way)
        # euclidean_matrix: [roi数 * resolution, way]
        metric_matrix = euclidean_matrix.reshape(box_per_image, resolution,
                                                 self.way)  # 包括了背景了, [roi数, resolution, way]
        metric_matrix = metric_matrix.mean(1)  # (roi数, way(背景))
        metric_matrix = self.compute_background(metric_matrix)
    ...
    def compute_background(self, metric_matrix):
        background_metrix = torch.ones_like(metric_matrix).to(metric_matrix.device) * 114514
        euclidean_matrix_mask = torch.where(metric_matrix > self.distance_threshold,
                                            background_metrix, metric_matrix)
        background_index = torch.where(euclidean_matrix_mask.mean(1) >= 114514)[0]
        foreground_index = torch.where(euclidean_matrix_mask.mean(1) < 114514)[0]
        assert len(background_index) + len(foreground_index) == metric_matrix.shape[0]
        n = metric_matrix.shape[0]
        bg = torch.zeros([n, 1]).to(metric_matrix.device)
        bg[foreground_index] = self.distance_threshold * 2
        metric_matrix = torch.cat([bg, metric_matrix], dim=1)
        tqdm.write('判断背景数/前景数:\t{:>6}/{:<6}, 背景阈值: {:.4}, 温度因子: {:.4}'.format(background_index.shape[0],
                                                                              foreground_index.shape[0],
                                                                              float(self.distance_threshold),
                                                                              float(self.scale.exp())))
        return metric_matrix


class FRPredictHead_FRDet(nn.Module):
    def __init__(self, way, shot, representation_size, Woodubry):
        super(FRPredictHead_FRDet, self).__init__()
        self.resolution = None
        self.way = way
        self.shot = shot
        self.representation_size = representation_size
        self.num_classes = way
        self.Woodubry = Woodubry
        self.bbox_pred = nn.Linear(representation_size, way * 4)
        self.encoder = nn.Sequential(nn.Conv2d(256, 64, kernel_size=1, stride=1, padding=0),
                                     # nn.BatchNorm2d(64),
                                     nn.LeakyReLU(inplace=True))
        self.r = nn.Parameter(torch.zeros(2), requires_grad=True)
        self.scale = nn.Parameter(torch.FloatTensor([5.0]), requires_grad=True)
        # self.gamma = nn.Parameter(torch.FloatTensor([-1.0]), requires_grad=True)
        self.distance_threshold = 0.5

    def forward(self, support, query, x):
        # self.distance_threshold = self.gamma.exp()
        # 回归
        if x.dim() == 4:
            assert list(x.shape[2:]) == [1, 1]
        x = x.flatten(start_dim=1)
        ...
        # 分类
        distance, support = self.cls_predictor(
            support=support,
            boxes_features=query,
            Woodubry=self.Woodubry)

        # distance [0, 无穷]
        distance = 2 * (torch.sigmoid(distance) - 0.5)  # [0, 1]
        self.compute_background(distance)
        distance = distance * self.scale.exp()  # [0, self.scale.exp()]
        score = 2 * torch.sigmoid(distance.neg())  # [-self.scale.exp(), 0] -> [0, 1]
        return score, bbox_deltas, support
    ...
    def cls_predictor(self, support: torch.Tensor, boxes_features: torch.Tensor, Woodubry=True):
        ...
        metric_matrix = self.metric(euclidean_matrix,
                                    box_per_image=roi_num,
                                    resolution=self.resolution)  # (roi数, way)
        logits = metric_matrix  # (roi数, way), 防止logits的数值过小导致softmax评分差距不大, 温度T
        return logits, support
    ...
    def compute_background(self, distance):
        """

        :param distance: [0, 1]
        :return: scores:[distance_threshold, 1]
        """
        bg_mask = torch.ones_like(distance)
        distance = torch.where(distance <= self.distance_threshold, distance, bg_mask)

        bg_inds = torch.where(distance.mean(1) == 1)[0]
        fg_inds = torch.where(distance.mean(1) < 1)[0]

        tqdm.write('判断背景数/前景数:\t{:>6}/{:<6}, 背景阈值: {:.4}, 温度因子: {:.4}'.format(bg_inds.shape[0],
                                                                              fg_inds.shape[0],
                                                                              float(self.distance_threshold),
                                                                              float(self.scale.exp())))
