...
if __name__ == '__main__':
    ...
    if not (os.path.exists('weights_coco_msf/results')):
        os.makedirs('weights_coco_msf/results')
    ...
    # ----------------------------------------
    if not os.path.exists('weights_coco_msf/results/loss_per_image.json'):
        with open('weights_coco_msf/results/loss_per_image.json', 'w') as f:
            json.dump({}, f)
            print('创建loss_per_image.json')
    if not os.path.exists('weights_coco_msf/results/loss_per_image_val.json'):
        with open('weights_coco_msf/results/loss_per_image_val.json', 'w') as f:
            json.dump({}, f)
            print('创建loss_per_image_val.json')

    # ----------------------------------------------------------------------------------------
    # continue_weight = 'weights_coco_msf/frnod5_40.pth'
    ...
    for e in range(0, epoch):  # 4 * 60000 * 8
        ...
        for i in range(num_mission):
            ...
            # print('将本次任务的loss写入weights_coco_msf/results/loss_per_image.json')
            with open('weights_coco_msf/results/loss_per_image.json', 'r') as f:
                content = json.load(f)
            with open('weights_coco_msf/results/loss_per_image.json', 'w') as f:
                content.update({'loss_epoch{}_mission_{}'.format(e + 1, i + 1): loss_list_tmp})
                json.dump(content, f)

            print('将本次任务的权重写入weights_coco_msf/frnod{}_{}.pth'.format(e + 1, i + 1))
            torch.save({'models': model.state_dict()}, 'weights_coco_msf/frnod{}_{}.pth'.format(e + 1, i + 1))
            ...
            # print('将本次任务的loss写入weights_coco_msf/results/loss_per_image_val.json')
            with open('weights_coco_msf/results/loss_per_image_val.json', 'r') as f:
                content = json.load(f)
            with open('weights_coco_msf/results/loss_per_image_val.json', 'w') as f:
        ...
        with open('weights_coco_msf/results/loss_bepoch{}.json'.format(e + 1), 'w') as f:
